...
const ContactList = ({ contacts , onUpdate, visibleContacts }) => {
            ...
            <Grid container spacing={3} style={{ marginBottom: '60px' }}>
              {visibleContacts.map((contact, index) => (
                <Grid item key={index} xs={12} md={6} lg={4}>
                  <ContactCard contact={contact} onUpdate={onUpdate} />
                </Grid>
              ))}
            </Grid>
        </Container>
        <Grid container justify="center" style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', zIndex: 1, backgroundColor: 'white', height: '50px', padding: '10px 0' }}>
          <Pagination
